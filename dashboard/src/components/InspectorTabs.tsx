  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto min-h-0">
        {needsGoal && !goalId ? (
          <div className="p-4 text-xs text-gray-400">{t("wsNoGoal")}</div>
        ) : tab === "diff" ? (
...
function WorkspaceFiles({ goalId }: { goalId: string }) {
  ...
  if (files === null) return <div className="p-4 text-xs text-gray-400">{t("loading")}</div>;
  if (files.length === 0) return <div className="p-4 text-xs text-gray-400">{t("wsNoFiles")}</div>;
  return (
    <div className="text-xs font-mono p-2">
      {files.map((f) => (
        <div key={f} className="px-2 py-0.5 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 truncate">
          {f}
        </div>
      ))}
    </div>
