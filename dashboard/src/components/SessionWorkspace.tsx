...
export function SessionWorkspace({
  ...
  /** 목록에서 지목한 태스크를 터미널에 물리고 곧바로 수임(claim)·착수 지시까지 보낸다. */
  const startTask = async (task: WorkspaceTask) => {
    ...
    try {
      ...
      const result = await api.terminals.claimNext(target.id, {
        taskId: task.id,
        goalId: task.goal_id,
        agentId: task.assignee_id ?? target.agentId ?? agentId ?? null,
