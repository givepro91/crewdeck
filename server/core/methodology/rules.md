...
## §10. 상시(standing) 서비스 보호 — 자기 종료 금지

에이전트가 도는 워크트리가 **상시 실행 중인 서비스와 같은 코드베이스**일 수 있다(dogfooding). 앱을 "실행"·검증하려고 그 상시 서비스를 죽이면 지금 이 오케스트레이션 자신이 종료된다.

- **금지**: `npm run dev`, `npm start`, `scripts/service-macos.sh`, `predev.sh`, `launchctl bootout`/`unload`, 고정 포트(예: :7200) 점유 프로세스 `kill` — 상시/production 서비스를 내리는 모든 명령.
- 앱 실행 검증이 필요하면 ① 이미 떠 있는 인스턴스를 사용하거나 ② **임시/격리 포트**로 새로 띄운다(고정 포트 재사용 금지). 그것도 안 되면 build + test + 정적 리뷰로 대체.
- 포트 충돌이 나도 상대 프로세스를 죽이지 말고 물러선다.
