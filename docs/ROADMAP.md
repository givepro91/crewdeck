...
- **최근 기능 마일스톤** (상세는 git history):
  - **Quality Gate bootstrap/upgrade 검증** — built CLI를 빈 임시 data directory와 격리 포트에서 실행해 정규화 table·column·index 자동 생성, dashboard asset, API 생성 goal의 빈 verification timeline을 관통 검증. legacy verification row를 보존한 upgrade와 동일 data directory 재시작의 migration idempotency/PID lock 정리도 `npm run verify:startup`으로 고정.
  ...
  - **goal 간 병렬 실행**(기본 동시성 2, goal 내부는 순차 1) + 검증 수렴 보장(fail 라운드 상한 → goal QA 이월) + 라이브 활동 흐름 타임라인 + 병렬 squash 충돌 integration-time 해결 파이프라인.
    - **스케줄링 계약 고정**(2026-07-11): 동시성을 agent/task 수가 아닌 활성 goal slot으로 계산. `CREWDECK_MAX_CONCURRENCY` 기본 2, goal 내 live task 1, spec/decompose lookahead 1. scheduler·수동 API는 같은 atomic task claim을 사용하고, `in_review`도 lane을 점유하여 Generator–Evaluator 분리 검증 전에 sibling이 출발하지 못함. 재시도/failover는 기존 goal slot에서 같은 task를 우선 재claim.
    - **자원 정리 계약 고정**(2026-07-11): 재시작 시 active task는 `todo`로 복구하고 active session/process와 stuck spec/squash 상태를 dispatch 전 정리하며, 활성 goal worktree는 보존. Goal 삭제 시 task cascade 삭제 후 spec/decompose/architect/evaluator/implementation session, scheduler flight·retry·failover ownership, checkpoint, worktree/branch를 정리. 실패 rollback과 squash-resolve 취소의 현재 예외는 Known Gaps에 명시. 세부 규범은 `docs/design/goal-as-unit.md#현행-스케줄링-계약`.
  ...
  - **"반영 차단"(goal squash) 근본 수정 + Quality Gate goal 수동 병합**(2026-07-11): 세 goal이 태스크 전부 done인데 main 반영이 막혀(`squash_status=blocked`) 있던 것을 진단. 원인은 goal squash 직전 WIP commit의 `git add -A -- . :(exclude).omc …`가 **최상위 `.omc`만 막고 중첩 `server/.omc`는 못 걸러** "paths are ignored" fatal을 낸 것(→ WIP commit 실패 → 반영 차단). 어제 `evaluator-diff` 테스트에서 격리한 global gitignore(`~/.gitignore_global`) 취약성의 **프로덕션 버전**. **수정**: exclude pathspec 제거 — 순수 `git add -A -- .`가 gitignore를 존중해 도구 경로를 자동 skip(staged 오염 0 실증). **반영**: 두 goal은 squash-approve의 자동 충돌 해결(base 전진 merge-in)로 반영, 세 번째 Quality Gate goal(3918줄·evaluator 대수술)은 main 15커밋 전진과 semantic 충돌해 **자동 해결 15분 타임아웃 → blocked**. 작업물이 `stash@{0}`에 안전함을 확인한 뒤 opus 서브에이전트가 goal의 판정·수정 루프 구조화 **+** main의 Scope Anchor 완화를 **둘 다 보존**해 수동 병합, 독립 검증(충돌마커 0·scope anchor 생존·typecheck·494 테스트) 후 squash. rebase 시 auto-fix 스톨 감지(`33bf323`)와도 충돌 → 양쪽 보존 해결. 검증: typecheck PASS·핵심 테스트 42/42·drain-safe 배포.
  - **goal squash 커밋명에 conventional prefix 자동 부착**(2026-07-11): crewdeck으로 만든 커밋에 `feat`/`fix` 같은 규칙이 없어 아쉽다는 피드백. goal 완료 요약(work-report)을 만드는 **기존 값싼 모델 1콜**에서 conventional type(feat/fix/update/docs/refactor/chore/test) 1개를 함께 뽑아 `work_report.commitType`으로 저장하고, `squash-approve`의 커밋 첫 줄 prefix로 사용(`goals.ts` 두 경로). **추가 LLM 콜 0.** 못 뽑거나 유효하지 않으면 `feat` 기본값, 요약 자체가 실패하면 prefix 없이 기존 동작(안전 degrade). 검증: typecheck PASS·work-report 테스트 12/12(신규 케이스 2).
  - **품질 판정 타임라인 task 그룹화 UI/UX 개편**(2026-07-11, "스크롤 심함 + 봐서 도움 되나" 피드백): auto-fix 루프가 같은 task를 반복 재시도해 라운드 행이 폭증(실측 goal 1개 33라운드)하던 것을 진단 — 라운드마다 전체 행 1개라 거의 다 "실패"인 행 33개가 스크롤을 채우고 정작 "지금 뭐가 막혔나"는 묻혔다. **수정**(`dashboard/src/components/GoalDetail.tsx` 단일 파일): ① 라운드를 `task_id` 단위로 그룹화 → **33행→5행**, ② task별 재시도 이력을 점 스트립(●빨강=실패/초록=통과)으로 압축해 "5번 빨강→초록(고쳐짐)" vs "7번 빨강(막힘)"이 한눈에, ③ 상단 요약(`3/5 통과·2 수정 중`)·현재 작업 중 task만 기본 펼침(파란 "수정 중" 뱃지)·이슈(왜 실패했나) 최상단·품질 점수 그리드·라운드 선택기, 세션ID 등 개발자 정보는 접이식 유지 → 개발자/비개발자 동시 만족. 검증: dashboard `tsc -b` PASS·해당 goal 실데이터(33라운드)를 격리 하버스에 마운트해 브라우저 렌더 실측(33→5·요약·점·이슈 정상)·`build:dashboard` 무중단 라이브 배포.
  - **overview 스크롤 절감 + 에이전트 팀 프레즌스 패널**(2026-07-11, "개요가 너무 길다 + 에이전트 요약이 버튼 벽 같다" 피드백): 개요 탭이 세로로 과적재(에이전트 요약 + 목표들 + 태스크 목록)돼 스크롤이 길던 것을 진단. 근본 원인은 각 `goal_as_unit` 카드가 `<GoalDetail>`(상태+품질 타임라인+활동)을 **항상 펼쳐** 렌더한 것. **수정**(`ProjectHome.tsx` + i18n): ① **목표 카드 진행 상세 기본 접힘** — `› 진행 상세` 토글, 접혀 있으면 GoalDetail 미마운트(불필요 fetch 제거) → 한 화면 목표 2~3개(이전 1개), ② **에이전트 요약 → 팀 프레즌스 패널** — 콤마 나열/중복(모든 에이전트 칩 + 별도 활동 목록)을 테두리 카드 하나로 통합: 헤더(인원·작업중 카운트·탭/복제 액션) + 작업 중 히어로 행(역할 아이콘 + 이름 + 실시간 활동) + 대기 아바타 클러스터(역할 이모지, 16개 초과 `+N`). `agent.role`(cto/pm/backend/frontend/ux/qa/reviewer/devops/marketer)별 아이콘·soft 컬러. 큐 컨트롤·한도 배너·승인 배지 등 동작은 전부 보존, 배치/접힘만 변경. 태스크 목록 접기(C)는 A로 병목 해소돼 보류. 검증: dashboard `tsc -b` PASS·라이브 overview 브라우저 실측(칩→패널·목표 접힘·토글 펼침)·`build:dashboard` 무중단 배포.
...
| Quality Gate (Generator-Evaluator 분리, 구조화 판정·fix loop·timeline) | 완성 + fresh/upgrade startup smoke (`evaluator.ts` + `verification-policy.ts` + `verify:startup`) |
...
| Goal-as-Unit 실패 rollback 미연결 | task 시작 checkpoint는 생성하지만 실행 예외 경로가 `restoreCheckpoint()`/`saveDiscardedDiff()`를 호출하지 않음. `blocked`/`todo` 재시도가 부분 실패 변경을 유지하므로 task 단위 rollback 계약 연결 필요 | Medium |
| Goal 삭제 시 squash resolver 미정리 | DELETE가 task/spec/decompose/architect/evaluator/implementation session은 종료하지만 `squash-resolve:${goalId}` session은 누락. resolving 중 삭제 시 세션 종료·후속 DB/WS 가드 필요 | Medium |
